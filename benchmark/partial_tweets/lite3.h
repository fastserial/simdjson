...
struct lite3 {
  ...
  bool run(simdjson::padded_string &json, std::vector<tweet<std::string_view>> &result) {
    ...
    while (lite3_iter_next(buf, buflen, &iter, NULL, &tweet_ofs) == LITE3_ITER_ITEM) {
      ...
      if (lite3_get(buf, buflen, tweet_ofs, "created_at", &val) < 0)
        return false;
      tweet.created_at = lite3_val_str(val);
      if (lite3_get_i64(buf, buflen, tweet_ofs, "id", (int64_t *)&tweet.id) < 0)
        return false;
      if (lite3_get(buf, buflen, tweet_ofs, "text", &val) < 0)
        return false;
      tweet.result = lite3_val_str(val);
      if (lite3_get_i64(buf, buflen, tweet_ofs, "in_reply_to_status_id", (int64_t *)&tweet.in_reply_to_status_id) < 0)
        tweet.in_reply_to_status_id = 0;
      
      size_t user_ofs;
      if (lite3_get_obj(buf, buflen, tweet_ofs, "user", &user_ofs) < 0)
        return false;
      if (lite3_get_i64(buf, buflen, user_ofs, "id", (int64_t *)&tweet.user.id) < 0)
        return false;
      if (lite3_get(buf, buflen, user_ofs, "screen_name", &val) < 0)
        return false;

      tweet.user.screen_name = lite3_val_str(val);
      if (lite3_get_i64(buf, buflen, tweet_ofs, "retweet_count", (int64_t *)&tweet.retweet_count) < 0)
        return false;
      if (lite3_get_i64(buf, buflen, tweet_ofs, "favorite_count", (int64_t *)&tweet.favorite_count) < 0)
        return false;
    }
...
BENCHMARK_TEMPLATE(partial_tweets, lite3)->UseManualTime();
